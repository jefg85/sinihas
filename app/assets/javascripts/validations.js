...
(function($) {
  $.fn.validate = function() {
    return this.filter('form[data-validate]').each(function() {
      ...
      var formularioId=form.attr('id');  

   
     var elementosRequeridos = $('*[require="true"]');
   	for (i=0; i<elementosRequeridos.length; i++){
   		$("label[for='"+elementosRequeridos[i].id+"']").addClass('required-field');
	}
      
    //validando campos requeridos
  	$('*[require="true"]').change(function(){  		  		
  		if($(this).val()==""){  			
  			  jQuery('label[for="' + this.id + '"]').addClass('label-error');
  			  jQuery(this).addClass('field-error');
  			  setToolTip('Requerido',false,'error',this);
  		}
  		else{   
  	    	jQuery('label[for="' + this.id + '"]').removeClass('label-error');
       ...
       //Agregando clase a label para campo requerido
      	jQuery.each(settings.validators, function(key, validator) {
    	  var elementName = key;
    	  var elementId = $('*[name="'+elementName+'"]').attr('id');
    	  for(v in validator){    		  
    		  if(v=='presence'){
    			$("label[for='"+elementId+"']").addClass('required-field');  
    		  }
    		  else if(v=='format'){
    			  if(validator[v]['keypress']){    				  
    				  $("#"+elementId).limitkeypress({ rexp: validator[v]['with'] });
    			  }
    			  if(validator[v]['mask']){ 
    				  $("#"+elementId).mask(validator[v]['maskformat']);    				      				  
    			  }
  ...
  formBuilders: {
    'ActionView::Helpers::FormBuilder': {
      add: function(element, settings, message) {    	  
    	  //agregando tooltip
    	  setToolTip(message,false,'error',element);
    	  jQuery('label[for="' + element.attr('id') + '"]').addClass('label-error');
    	  jQuery('#' + element.attr('id')).addClass('field-error');  
      ...
      remove: function(element, settings) {    	 
    	...
    	jQuery('label[for="' + element.attr('id') + '"]').removeClass('label-error');
  	  	jQuery('#' + element.attr('id')).removeClass('field-error');  	    	  
